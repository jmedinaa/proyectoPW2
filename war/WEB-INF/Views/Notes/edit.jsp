...
<% List<User> array = (List<User>)request.getAttribute("students"); %>
<% Note note= (Note) request.getAttribute("note"); %>
<% List<Course> array2 = (List<Course>)request.getAttribute("courses"); %>
<!DOCTYPE HTML>
<html>
<head>
...
<body>
	...
	<div class="container-fluid text-center">    
	  <div class="row content">
	    ...
	    <br>
	      <h3 class="text-primary">Notes Edit</h3>
		  <form method="post"  action="/notes/edit">
		   <div class="form-group">
			    <label>Id:</label>
			    <input type="text" class="form-control" readonly name="id"  value="<%=note.getId()%>">
		   </div>
		   <div class="form-group">
			   <label>Student:</label>
			    <select class="form-control"  name="idAlum" required>
			    <%if(array.size() > 0) {
					for(User alum : array){ 
					if(alum.getId()==note.getIdStudent()){%>
				      <option value="<%=alum.getId()%>" selected><%=alum.getId()%></option>
				      <%}
				   else{%>
					   <option value="<%=alum.getId()%>" ><%=alum.getId()%></option>
				<%  }}}%>
				</select>
		  </div>
		   <div class="form-group">
			   <label>Course:</label>
			    <select class="form-control"  name="idCourse" required>
			    <%if(array2.size() > 0) {
					for(Course course : array2){ 
					if(course.getId()==note.getIdCourse()){%>
				      <option value="<%=course.getId()%><%=course.getName()%>" selected><%=course.getName()%></option>
				      <%}
				   else{%>
					   <option value="<%=course.getId()%><%=course.getName()%>" ><%=course.getName()%></option>
				<%  }}}%>
				</select>
		  </div>
		   <div class="form-group">
			    <label>Note:</label>
			    <input type="number" name="note" class="form-control" value ="<%=note.getNoteValue()%>"placeholder="Nota" min="0" max="20" required autofocus>
		   </div>
