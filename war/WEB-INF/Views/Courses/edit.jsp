...
<% List<User> array = (List<User>)request.getAttribute("teachers"); 
	  ...
	  <div class="row content">
	    <div class="col-sm-2 sidenav">
	       <div class="navbar-nav mr-auto  text-left">
				    ...
				    <a class="nav-item nav-link" href="/courses">List Courses</a>
			</div>
	    </div>
	    ...
	    <br>
		  ...
		  <form method="post"  action="/courses/edit">
		   <div class="form-group">
			    <label>Id:</label>
			    <input type="text" class="form-control" readonly name="id"  value="<%=course.getId()%>">
		   </div>
		    <div class="form-group">
			    <label>Name:</label>
			    <input type="text" class="form-control" name="name" value="<%=course.getName()%>" placeholder="Name of Course" required autofocus>
		   </div>
		   <div class="form-group">
			   <label>Teachers:</label>
			    <select class="form-control"  name="idTeacher" required>
			      <%if(array.size() > 0) {
					for(User teacher : array){ 
					if(teacher.getId()==course.getIdTeacher()){%>
				      <option value="<%=teacher.getId()%><%=teacher.getEmail()%>" selected><%=teacher.getEmail()%></option>
				      <%}
				   else{%>
					   <option value="<%=teacher.getId()%><%=teacher.getEmail()%>" ><%=teacher.getEmail()%></option>
				<%  }}}%>
				</select>
		  </div>
		   <div class="form-group">
